...
|--------------|-------------------|--------------|--------------------------|  
|AjaxSession-V8|Web Control|Yes|Any time a session that uses the Bing Maps Version 8.0 web control, is started with a valid [!INCLUDE[maps_ticket](../articles/includes/maps-ticket-md.md)], one (1) transaction is counted. A session begins with the load of the Bing Maps Web Control into a user’s browser and ends when the browser is closed or the user moves to a different page.<br /><br /> Anytime the Autosuggest Module is used without a map and a user selects a location suggestion from the menu one (1) transaction is counted.  When using the AutoSuggest Module, transactions are not logged for user key strokes or based on the number of location results that are returned in the Autosuggest menu.<br /><br /> All REST or Spatial Data<sup>2</sup> service requests that use a [session ID](../getting-started/understanding-bing-maps-transactions.md#sessionkey) are **not** billable when they are made from within a session.|  
|Win10-Native|Windows 10 UWP Map control|Yes|Any time an application session that the Windows 10 UWP Map control is started with a valid [!INCLUDE[maps_ticket](../articles/includes/maps-ticket-md.md)], one (1) transaction is counted. An application session begins with the load of the Windows 10 UWP Map control in an application and ends when the application is closed.|  
|WPFSession|WPF Control|Yes|Any time an application session is started and uses the [!INCLUDE[wpf_product_name](../getting-started/includes/wpf-product-name-md.md)] with a valid [!INCLUDE[maps_ticket](../articles/includes/maps-ticket-md.md)], one (1) transaction is counted. An application session begins with the load of the Bing Maps WPF Control by the application and ends when the application is closed.<br /><br /> All SOAP, REST or Spatial Data<sup>2</sup> service requests that use a [session ID](../getting-started/understanding-bing-maps-transactions.md#sessionkey) are **not** billable when they are made from within a session.|  
...
|Route-DistanceMatrix|REST Services|Yes|Any time a synchronous [Distance Matrix](https://msdn.microsoft.com/library/mt827298.aspx) request is made, one (1) transaction is counted for every two (2) cells calculated in the matrix.|  
|Route-DistanceMatrixAsync|REST Services|Yes|Any time an asynchronous [Distance Matrix](https://msdn.microsoft.com/library/mt827298.aspx) request is made, one (1) transaction is counted for every two (2) cells calculated in the matrix.|  
|Route-DistanceMatrixAsyncCallback|REST Services|No|Any time the status of an asynchronous [Distance Matrix](https://msdn.microsoft.com/library/mt827298.aspx) request is checked.|  
|Route-Isochrone|REST Services|Yes|Any time a synchronous [Isochrone](https://msdn.microsoft.com/library/mt814921.aspx) request is made, one (1) transaction is counted.|  
|Route-IsochroneAsync|REST Services|Yes|Any time an asynchronous [Isochrone](https://msdn.microsoft.com/library/mt814921.aspx) request is made, one (1) transaction is counted.|  
|Route-IsochroneAsyncCallback|REST Services|No|Any time the status of an asynchronous [Isochrone](https://msdn.microsoft.com/library/mt814921.aspx) request is checked.|  
|Route-SnapToRoad|REST Services|Yes|Any time a synchronous [Snap to Road](https://msdn.microsoft.com/library/mt814926.aspx) request is made, one (1) transaction is counted.|  
|Route-SnapToRoadAsync|REST Services|Yes|Any time an asynchronous [Snap to Road](https://msdn.microsoft.com/library/mt814926.aspx) request is made, one (1) transaction is counted.|  
|Route-SnapToRoadAsyncCallback|REST Services|No|Any time the status of an asynchronous [Snap to Road](https://msdn.microsoft.com/library/mt814926.aspx) request is checked.|  
|Route-Truck|REST Services|Yes|Any time a synchronous [Truck Routing](https://msdn.microsoft.com/library/mt814923.aspx) request is made, three (3) transactions are counted.|  
|Route-TruckAsync|REST Services|Yes|Any time an asynchronous [Truck Routing](https://msdn.microsoft.com/library/mt814923.aspx) request is made, three (3) transactions are counted.|  
|Route-TruckAsyncCallback|REST Services|No|Any time the status of an asynchronous [Truck Routing](https://msdn.microsoft.com/library/mt814923.aspx) request is checked.|  
...
|Category|Bing Maps API|Billable|Category Description|  
|--------------|-------------------|--------------|--------------------------|  
|Dataflow:BatchGeocode|Spatial Data Services|No<sup>4,6</sup>|Any time a request is made to [Create Job](../spatial-data-services/create-a-geocode-job-and-upload-data.md), a set of transactions equal to the number of total entities in the geocode request are counted.<br /><br /> Any time a data source is geocoded using the [!INCLUDE[maps_devportal_name](../getting-started/includes/maps-devportal-name-md.md)], a set of transactions equal to the number of total entities in the geocode request are counted.|  
|Dataflow:Create|Spatial Data Services|No<sup>6</sup>|Any time a URL request is made to create a dataflow job to batch geocode entity data by using the [Create Job](../spatial-data-services/create-a-geocode-job-and-upload-data.md) API, one (1) transaction is counted.<br /><br /> Any time a URL request is made to stage, publish or rollback a data source by using the [Create a Load Data Source Job](../spatial-data-services/create-a-load-data-source-job-and-input-entity-data.md) API, one (1) transaction is counted.<br /><br /> Any time a data source is geocoded or published by using the [!INCLUDE[maps_devportal_name](../getting-started/includes/maps-devportal-name-md.md)], one (1) transaction is counted.|  
|Dataflow:Get|Spatial Data Services|No<sup>6</sup>|Any time a URL request is made to [Get Job Status](../spatial-data-services/get-status-of-a-geocode-job.md), one (1) transaction is counted.<br /><br /> Any time a URL request is made to [Get Load Data Source Status](../spatial-data-services/get-load-data-source-status.md), one (1) transaction is counted.<br /><br /> Any time a URL request is made to [Get Download Data Source Job Status](../spatial-data-services/get-download-data-source-job-status.md), one (1) transaction is counted.<br /><br /> Any time a data source is geocoded or published by using the [!INCLUDE[maps_devportal_name](../getting-started/includes/maps-devportal-name-md.md)], one (1) transaction is counted.|  
|Dataflow:Download|Spatial Data Services|No<sup>6</sup>|Any time a URL request is made to [Download Results](../spatial-data-services/download-geocode-job-results.md), one (1) transaction is counted.<br /><br /> Any time the geocoded results of non-published data source entity data are downloaded using the [!INCLUDE[maps_devportal_name](../getting-started/includes/maps-devportal-name-md.md)], one (1) transaction is counted.<br /><br /> Note that if you download successful and failed results, these are two separate **Dataflow:Download** transactions.|  
...
|RESTSpatialDataService:DownloadDatasource|Spatial Data Services|No<sup>6</sup>|Any time a URL request is made to [Create a Download Job](../spatial-data-services/create-a-download-job.md), one (1) transaction is counted.<br /><br /> Any time a URL request is made to [Get Downloaded Data](../spatial-data-services/get-downloaded-data.md), one (1) transaction is counted.<br /><br /> Any time a published, staged or previous version of a data source is downloaded using the [!INCLUDE[maps_devportal_name](../getting-started/includes/maps-devportal-name-md.md)], one (1) transaction is counted.|  
|RESTSpatialDataService:DeleteDataSource|Spatial Data Services|No<sup>6</sup>|Any time a URL request is made to [Delete a Data Source](../spatial-data-services/delete-a-data-source.md), one (1) transaction is counted.<br /><br /> Any time a data source is deleted using the [!INCLUDE[maps_devportal_name](../getting-started/includes/maps-devportal-name-md.md)], one (1) transaction is counted.|  
...
|RESTSpatialDataService:Query|Spatial Data Services|Yes<sup>5,6</sup>|Any time a [Query API](../spatial-data-services/query-api2.md) URL request is made to query a data source, one (1) transaction is counted.<br /><br /> When a [Query by Area](../spatial-data-services/query-by-area.md) URL request is made with an address string that must be geocoded, one RESTLocations transaction is also counted.|  
|RESTSpatialDataService:Geodata|Spatial Data Services|No<sup>6</sup>|Any time a [Geodata API](../spatial-data-services/geodata-api1.md) URL request is made to get boundary data, one (1) transaction is counted.<br /><br /> When a [Geodata API](../spatial-data-services/geodata-api1.md) URL request is made with an address string that must be geocoded, one RESTLocations transaction is also counted.|  
  
 <sup>4</sup>Spatial Data Services batch geocode transactions are free up to one (1) million transactions per year for a [!INCLUDE[ve_platform_dev_acct](../getting-started/includes/ve-platform-dev-acct-md.md)]. After you reach one (1) million transactions in a year, these transactions become billable.  
  
 <sup>5</sup>This transaction is not billable if the service request is made using a [session ID](../getting-started/understanding-bing-maps-transactions.md#sessionkey) from an Web or WPF Control session, and the total number of Spatial Data Services transactions is less that ten (10) million per year for a [!INCLUDE[ve_platform_dev_acct](../getting-started/includes/ve-platform-dev-acct-md.md)].  
  
 <sup>6</sup>Spatial Data Services requests become billable when you reach ten (10) million transactions within a year for a [!INCLUDE[ve_platform_dev_acct](../getting-started/includes/ve-platform-dev-acct-md.md)] even when a session ID is used.  
  
### Discontinued Map Control APIs (AJAX V7, Silverlight, Windows Store)  
  
|Category|Bing Maps API|Billable|Category Description|  
|--------------|-------------------|--------------|--------------------------|  
|AjaxSession|AJAX Control (DISCONTINUED)|Yes|Any time a session that uses [!INCLUDE[vemc_product_name](../articles/includes/vemc-product-name-md.md)] or Version 7.0, is started with a valid [!INCLUDE[maps_ticket](../articles/includes/maps-ticket-md.md)], one (1) transaction is counted. A session begins with the load of the Bing Maps AJAX Control into a user’s browser and ends when the browser is closed or the user moves to a different page.|  
|WinStoreAppSession_JavaScript_V1<br /><br /> <br /><br /> WinStoreAppSession_JavaScript (for Windows 8 Beta and Preview versions)|Windows Store apps (DISCONTINUED)|Yes|Any time a session that uses Bing Maps SDK for Windows Store apps using JavaScript  is started with a valid [!INCLUDE[maps_ticket](../articles/includes/maps-ticket-md.md)], one (1) transaction is counted. A session begins with the load of the Bing Maps for JavaScript control into a user’s browser and ends when the browser is closed or the user moves to a different page.|  
|WinStoreAppSession_CSharp_VB_CPP_V1<br /><br /> <br /><br /> WinStoreAppSession_CSharp_VB_CPP (Windows 8 Beta and Preview versions)|Windows Store apps (DISCONTINUED)|Yes|Any time an application session that uses Bing Maps SDK for Windows Store apps for C#, C++, or Visual Basic  is started with a valid [!INCLUDE[maps_ticket](../articles/includes/maps-ticket-md.md)], one (1) transaction is counted. An application session begins with the load of Bing Maps for Windows Store apps for C#, C++, or Visual Basic control in an application and ends when the application is closed.|  
|SilverlightSession<br /><br /> <br /><br /> SilverlightSessionBeta (for beta release of the control)|Silverlight Control (DISCONTINUED)|Yes - SilverlightSession<br /><br /> <br /><br /> No – SilverlightSessionBeta|Any time a session that uses the [!INCLUDE[sl_product_name](../getting-started/includes/sl-product-name-md.md)] is started with a valid [!INCLUDE[maps_ticket](../articles/includes/maps-ticket-md.md)], one (1) transaction is counted. A session begins with the load of the Bing Maps Silverlight Control into a user’s browser and ends when the browser is closed or the user moves to a different page.|  
...
|iOSSession|iOS Control (DISCONTINUED)|Yes|Any time an application session is started and uses the [!INCLUDE[bm_ios_control](../getting-started/includes/bm-ios-control-md.md)] with a valid [!INCLUDE[maps_ticket](../articles/includes/maps-ticket-md.md)], one (1) transaction is counted. An application session begins with the load of the [!INCLUDE[bm_ios_control](../getting-started/includes/bm-ios-control-md.md)] by the application and includes all [!INCLUDE[bm_ios_control](../getting-started/includes/bm-ios-control-md.md)] interactions until the application is closed.|  
