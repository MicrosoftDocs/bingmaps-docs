...
> Infobox(location: [Location](Location%20Class.md), options?: [InfoboxOptions](InfoboxOptions%20Object.md))

When creating an infobox a location must be passed as an argument to the constructor. Optionally, [infobox options](InfoboxOptions%20Object.md) can also be passed in as an argument.
...
`getAnchor()`                      | [Point](Point%20Class.md)                 | Returns the point on the infobox which is anchored to the map. An anchor of (0,0) is the top left corner of the infobox.
...
`getLocation()`                    | [Location](Location%20Class.md)              | Returns the location on the map where the infobox’s anchor is attached.
...
`getOffset()`                      | [Point](Point%20Class.md)                 | Returns the amount the infobox pointer is shifted from the location of the infobox, or if showPointer is false, then it is the amount the infobox bottom left edge is shifted from the location of the infobox. The default value is (0,0), which means there is no offset.
`getOptions()`                     | [InfoboxOptions](InfoboxOptions%20Object.md)       | Returns the infobox options.
...
setLocation(loc: [Location](Location%20Class.md))       |                       | Sets the location on the map where the anchor of the infobox is attached.
setMap(map: [Map](Map%20Class.md))| | Adds the infobox to the map. To remove an Infobox from the map, simply pass null into this function. **Note**: Calling this function removes all event handlers from the infobox. Add your event handlers to the infobox after calling this function. |
setOptions(opt: [InfoboxOptions](InfoboxOptions%20Object.md), ignoreDelay?: boolean)	 |                       | Sets options for the infobox. If the `ignoreDelay` parameter is set to true, the `closeDelayTime` option will be ignored if the visible option is being changed from true to false.
