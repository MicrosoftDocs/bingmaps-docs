...
>     if self.mapView.try(toConvertOffset: point, into: &location) {
...
The following example shows how to change [MapIcon]() image and z index of containing layer when a pusphin is tapped.

At first, add extension for [MSMapElementCollection]() to conform to the Sequence Protocol in order to use for-in loop. 
