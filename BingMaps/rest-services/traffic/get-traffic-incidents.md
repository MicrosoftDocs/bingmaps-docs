...
|---------------|-----------|-----------------|------------|  
|`mapArea`||**Required.** Specifies the area to search for traffic incident information.|A rectangular area specified as a bounding box. The height or width of the area bounding box should not exceed 500km. If either the width or height of the bounding box exceeds 500km the request may time out. Note that a maximum limit of 30 degrees for either width or height is enforced, requests with such bounding boxes will fail with HTTP status code 400.<br /><br /> A bounding box defines an area by specifying SouthLatitude, WestLongitude, NorthLatitude, and EastLongitude values. For more information, see [Location and Area Types](../common-parameters-and-types/location-and-area-types.md).<br /><br /> **Example**: 45.219,-122.325,47.610,-122.107|  
