...
| `routeAttributes`   | `ra`  | **Optional.** Specify to include or exclude parts of the routes response. Can be one of the following values: <br/><br/> • **routePath**: Include a set of point (latitude and longitude) values that describe the route’s path in the response.<br/><br/>**Example**: routeAttributes=routePath <br/><br/>• **regionTravelSummary**: Include travel summary of distance and time by two entity types: country (e.g. US, Canada) and administrative division or subregion (e.g. “state” in US and “province” in Canada). This value can be specified along with all the other routeAttributes values above.|
