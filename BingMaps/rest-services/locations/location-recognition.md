...
|`verboseplacenames`||**Optional.** Either `true` or `false`.<br><br/>- If `false`: "admin1" & country/region names will be in their official form (e.g.: “WA” for Washington state in USA and full name, e.g. "Occitanie" in France).<br/><br/>-If `true`: "admin1" names will be in expanded form (e.g. Washington), and "countryRegion" field with full name is added to address.|Default value is `false`.| 
