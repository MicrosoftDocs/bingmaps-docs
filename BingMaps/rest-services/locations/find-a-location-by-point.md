...
|`verboseplacenames`|`vbpn`|**Optional.** Either `true` or `false`<br><br/>- If `false`: "AdminDistrict" names will be in the officially abbreviated format (e.g.: “WA” for Washington state in USA).<br/><br/>-If `true`: "AdminDistrict" names will be in expanded form (e.g.: "Washington" instead of "WA" in the USA).|Default value is `false`.|
